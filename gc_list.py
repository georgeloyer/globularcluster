...
if online == True:
    try:
        ...
        path = os.path.abspath(inspect.getfile(inspect.currentframe())).replace(filename, 'gc_cat.fits')
        gc_cat = fits.open(path)[1].data

if online == False:
    path = os.path.abspath(inspect.getfile(inspect.currentframe())).replace(filename,'gc_cat.txt')
    #gc_cat = fits.open(path)[1].data
    gc_cat = ascii.read(path)
...
found_flg = False
print("Number of entries in catalog: ", len(gc_cat))
for i in range(len(gc_cat)):
    print(gc_cat['Name'][i], "\t", gc_cat['Alt.Name'][i], "\t", gc_cat['R_galcen'][i])
print("")
print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
print("")
sys.exit()
...
for i in range(len(gc_cat)):
    if gc_cat['Alt.Name'].mask[i]:
        altname = ''
    else:
        altname = gc_cat['Alt.Name'][i].lower()

    if query == gc_cat['Name'][i].lower() or query == altname:
        found_flg = True
        print('=============================================================================================')
        ...
        print('Predicted NH based on extinction (Bahramian+2015):','{:0.2e}'.format(gc_cat['Reddening'][i]*3.1*2.81e21),'cm^-2')
